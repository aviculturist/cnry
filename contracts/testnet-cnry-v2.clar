...
(define-public (watch (tokenId uint))
  (begin
    ...
    (unwrap! (contract-call? .watcher-v2 watch-token "cnry-v2" tokenId tx-sender ) (err ERR_CONTRACT_CALL))
    (ok block-height)
  )
)
...
(define-public (migrate-v2)
  (let
    (
      ;; testnet is indexed at 0
      (zero (unwrap! (contract-call? .cnry get-metadata u0) (err ERR_CONTRACT_CALL)) )
      (one (unwrap! (contract-call? .cnry get-metadata u1) (err ERR_CONTRACT_CALL)) )
      (two (unwrap! (contract-call? .cnry get-metadata u2) (err ERR_CONTRACT_CALL)) )
      (three (unwrap! (contract-call? .cnry get-metadata u3) (err ERR_CONTRACT_CALL)) )
    )
    ...
    (begin
      ...
      (unwrap! (as-contract (contract-call? .watcher-v2 add-watched-contract))  (err ERR_WATCHED_CONTRACT))
      (ok block-height)
    )
  )
)
